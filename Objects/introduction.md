...
Properties are accessible using the dot notation (e.g. `user.name`) and can be added, removed and read at any time. The last property may end with a trailing comma.

To remove a property, use a `delete` operator (e.g. `delete user.age;`).
